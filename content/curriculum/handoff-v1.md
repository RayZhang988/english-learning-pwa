...
3. 从学习单元字段投影 04 的 `LearningCandidate`；只在运行时计算
   `prerequisitesMet`。
4. 训练模块只消费 `LearningTask.targetModuleId` 与本模块相同的任务。
5. 完成、暂停、跳过和不可评分结果按 04 的 v1 事件契约上报。
6. 未识别 activity、损坏 `contentRef` 或缺失资源必须按内容故障处理，不能算学习者
   做错。
7. `recommendedDay` 是解锁顺序，不是固定每日计划；不得绕开 04 自己拼 45 分钟课表。
...
5. 每个单元 `estimatedSeconds === 900`，每个内容块合计 2700 秒。
...
28 个测试文件通过
89 项测试通过
lint、TypeScript、Vite build、PWA 生成通过
```
