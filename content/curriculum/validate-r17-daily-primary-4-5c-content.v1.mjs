...
const batch = items.filter((item) => item.id.startsWith('r17-daily-p4c-'))
assert.equal(batch.length, 55)
for (const level of [0, .5, 1, 1.5, 2]) assert.equal(items.filter((item) => difficulty(item) === level).length, 200)
for (const prefix of ['p4a', 'p4b']) assert.equal(items.filter((item) => item.id.startsWith(`r17-daily-${prefix}-`)).length, 55)
for (const item of batch) {
  assert.equal(item.growthDifficultyLevel, 2)
  assert.match(item.dailyKnowledgeId, /^daily-knowledge-v1:p4c:[0-9]{3}$/)
...
console.log('R17 daily primary-4 5C content quality verified')
