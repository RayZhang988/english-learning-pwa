...
const batch = items.filter((item) => item.id.startsWith('r17-daily-p6a-'))
...
assert.equal(items.filter((item) => difficulty(item) === 3).length, 92)
for (const item of batch) { assert.equal(item.growthDifficultyLevel,3);assert.match(item.dailyKnowledgeId,/^daily-knowledge-v1:p6a:[0-9]{3}$/);assert.ok(item.term.trim()&&item.meaningZh.trim()&&item.exampleEn.trim()&&item.exampleZh.trim());assert.ok(!/[；;/／]/u.test(item.meaningZh));assert.ok(normalize(item.exampleEn).includes(normalize(item.term)));assert.equal(items.filter((candidate)=>normalize(candidate.term)===normalize(item.term)).length,1);assert.ok(!('sceneKnowledgeId'in item)&&!('sceneId'in item)) }
console.log('R17 daily primary-6 7A content quality verified')
