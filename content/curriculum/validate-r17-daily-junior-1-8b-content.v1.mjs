...
const batch = items.filter((item) => item.id.startsWith('r17-daily-j1b-'))
...
assert.equal(items.filter((item) => level(item) === 3.5).length, 137)
for (const item of batch) {
  assert.equal(item.growthDifficultyLevel, 3.5)
  assert.match(item.dailyKnowledgeId, /^daily-knowledge-v1:j1b:[0-9]{3}$/)
...
console.log('R17 daily junior-1 8B content quality verified')
