...
const batch = items.filter((item) => item.id.startsWith('r17-daily-k1b-'))
...
assert.equal(batch.length, 75, 'Kindergarten 1B must contain exactly 75 daily knowledge points.')
...
for (const item of batch) {
  ...
  assert.match(item.dailyKnowledgeId, /^daily-knowledge-v1:k1b:[0-9]{3}$/)
...
console.log('R17 daily kindergarten 1B content quality verified')
