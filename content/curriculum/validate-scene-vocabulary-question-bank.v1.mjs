...
for (const [position, scene] of bank.scenes.entries()) {
  ...
  assert.equal(scene.questions.length, index.coverage.questionsPerScene)
  for (const question of scene.questions) {
    questionCount += 1
    assert.match(question.questionId, new RegExp(`^r13b-vocabulary-${sceneId}-q[0-9]{2}$`))
    ...
    assert.match(question.targetText, /^\S+$/u)
    ...
    assert.match(question.source.sourceId, new RegExp(`^r13b-source-${sceneId}-q[0-9]{2}$`))
...
assert.equal(seenQuestionIds.size, 108)
assert.equal(seenSourceIds.size, 108)
