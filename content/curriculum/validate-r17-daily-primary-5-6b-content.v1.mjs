...
const batch = items.filter((item) => item.id.startsWith('r17-daily-p5b-'))
...
assert.equal(items.filter((item) => difficulty(item) === 2.5).length, 144)
for (const item of batch) {
  assert.equal(item.growthDifficultyLevel, 2.5)
  assert.match(item.dailyKnowledgeId, /^daily-knowledge-v1:p5b:[0-9]{3}$/)
...
console.log('R17 daily primary-5 6B content quality verified')
