...
export function GrowthUpgradeRouteHost() {
  ...
  const saveDraft = (nextDraft: string) => {
    setDraft(nextDraft); setSavingDraft(true); setError(null)
    void coordinator.growth.saveUpgradeDraft({ eventId: `growth-draft:${domain}:${crypto.randomUUID()}`, domain, index: view.index, draft: nextDraft, savedAt: new Date().toISOString() }).catch((cause) => {
      setError(cause instanceof Error ? cause : new Error('草稿保存失败。'))
  ...
  return <main className="training-shell" aria-label={`${readableDomain(domain)}升级测试`}>
    ...
    <section className="training-card">
      {question.domain === 'vocabulary' ? <VocabularyQuestion question={question.question} draft={draft} setDraft={saveDraft} disabled={busy || feedback !== null} /> : null}
      {question.domain === 'listening' ? <ListeningQuestion question={question.question} draft={draft} setDraft={saveDraft} disabled={busy || feedback !== null} /> : null}
