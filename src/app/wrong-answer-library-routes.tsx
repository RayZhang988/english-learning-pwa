...
import { DictationAnswerGuidance, WrongAnswerLibraryScreen, WrongAnswerReviewScreen, type WrongAnswerLibraryTab, type WrongAnswerReviewViewModel } from '../ui/index.ts'
...
export function WrongAnswerReviewRouteHost() {
  ...
  if (active?.kind === 'listening') slot = { kind: 'listening', content: <ListeningReviewQuestionSlot active={active} coordinator={coordinator} phase={currentPhase} disabled={disabled} /> }
  if (active?.kind === 'speaking') slot = { kind: 'speaking', content: <SpeakingReviewQuestionSlot active={active} coordinator={coordinator} disabled={snapshot.busy || currentPhase === 'feedback'} /> }
