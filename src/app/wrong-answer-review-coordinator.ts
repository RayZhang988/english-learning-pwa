...
export class WrongAnswerReviewCoordinator {
  ...
  async selectVocabulary(optionId: string) { const active = this.snapshot.active; if (active?.kind !== 'vocabulary' || !active.question.options.some((option) => option.id === optionId)) throw new TypeError('Vocabulary option is unavailable.'); const library = await this.#options.state.update((latest) => { const round = assertRecoverableWrongAnswerReviewRound(latest); if (!round || round.stage !== 'answering' || round.order[round.index] !== active.record.recordId) throw new TypeError('Stale vocabulary review interaction.'); return updateWrongAnswerReviewRoundSnapshot(latest, { ...round, answerDraft: `${VOCABULARY_DRAFT_PREFIX}${optionId}`, updatedAt: this.#now() }) }); return this.#set({ ...this.snapshot, library, active: { ...active, selectedOptionId: optionId } }) }
  ...
  async listening(action: 'toggle' | 'retry') { if (this.#runtime?.kind !== 'listening') throw new TypeError('Listening review is unavailable.'); const runtime = this.#runtime.value; if (action === 'retry') await runtime.retryPlayback(); else await runtime.togglePlayback(); return this.#refreshActive() }
  async setListeningRate(rate: number) { if (this.#runtime?.kind !== 'listening') throw new TypeError('Listening review is unavailable.'); await this.#runtime.value.setRate(rate); return this.#refreshActive() }
  async setListeningRepeat(mode: ListeningWrongAnswerReviewSnapshot['playback']['repeatMode']) { if (this.#runtime?.kind !== 'listening') throw new TypeError('Listening review is unavailable.'); await this.#runtime.value.setRepeatMode(mode); return this.#refreshActive() }
  async selectListeningSegment(segmentId: string) { if (this.#runtime?.kind !== 'listening') throw new TypeError('Listening review is unavailable.'); await this.#runtime.value.selectSegment(segmentId); return this.#refreshActive() }
  async selectListening(optionId: string) { if (this.#runtime?.kind !== 'listening') throw new TypeError('Listening review is unavailable.'); await this.#runtime.value.select(optionId); return this.#refreshActive() }
  async changeListening(value: string) { if (this.#runtime?.kind !== 'listening') throw new TypeError('Listening review is unavailable.'); await this.#runtime.value.changeDictation(value); return this.#refreshActive() }
  ...
  async startSpeaking() { if (this.#runtime?.kind !== 'speaking') throw new TypeError('Speaking review is unavailable.'); await this.#runtime.value.startRecording(); return this.snapshot }
  async stopSpeaking() { const active = this.snapshot.active; if (active?.kind !== 'speaking' || this.#runtime?.kind !== 'speaking' || !this.snapshot.library?.activeRound) throw new TypeError('Speaking review is unavailable.'); await this.#runtime.value.stopRecording(`speaking-review:${this.snapshot.library.activeRound.roundId}:${active.record.recordId}`, this.#now()); return this.snapshot }
  async playSpeaking() { if (this.#runtime?.kind !== 'speaking') throw new TypeError('Speaking review is unavailable.'); await this.#runtime.value.playRecording(); return this.snapshot }
