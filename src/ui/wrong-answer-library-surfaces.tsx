...
export function WrongAnswerLibraryScreen({ viewModel, onExit, onSwitchTab, onStartRound, onResumeRound, onRetry }: WrongAnswerLibraryScreenProps) {
  return <main className="wrong-answer-library" aria-busy={viewModel.status === 'loading' || viewModel.busy}>
    ...
    {viewModel.status === 'error' ? <ErrorState title={viewModel.error?.title ?? '错题库暂时无法打开'} description={viewModel.error?.description ?? '请稍后重试。'} onRetry={onRetry} /> : null}
