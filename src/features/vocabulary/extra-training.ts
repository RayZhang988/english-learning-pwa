...
export class ExtraVocabularyTrainingRuntime {
  ...
  private async save(snapshot: ExtraVocabularyTrainingSnapshot) { const previous = this.snapshot; this.snapshot = snapshot; try { await this.repository.save(snapshot); return snapshot } catch (error) { if (this.snapshot === snapshot) this.snapshot = previous; throw error } }
  ...
  submit() { return this.queue(async () => { const snapshot = this.require(); if (snapshot.phase !== 'answering' || !snapshot.question || snapshot.selectedOptionId === null) throw new VocabularyError('session-transition-invalid', 'Select an answer before submitting.'); await this.timing?.transition({ phase: 'feedback', reason: 'active-feedback' }); const now = this.now(); let pendingWrongAnswerEvidence = snapshot.pendingWrongAnswerEvidence ?? []; if (this.options.wrongAnswerReview && snapshot.activeItem && !judgeVocabularyAnswer(snapshot.question, snapshot.selectedOptionId)) { const identity = await this.options.wrongAnswerReview.identityForItem(snapshot.activeItem); pendingWrongAnswerEvidence = [...pendingWrongAnswerEvidence, createVocabularyWrongAnswerEvidence({ identity, source: 'extra-training', taskOrSessionId: snapshot.session.sessionId, questionId: snapshot.question.id, submittedAt: now, correct: false })] } return this.flushWrongAnswerEvidence(await this.save({ ...snapshot, phase: 'feedback', pendingWrongAnswerEvidence, updatedAt: now })) }) }
  ...
  private async completeCurrentItemNow(snapshot: ExtraVocabularyTrainingSnapshot): Promise<ExtraVocabularyTrainingSnapshot> {
    ...
    const supplyRound = snapshot.session.supplyRound === undefined
      ? undefined
      : recordTrainingSupplyItem(snapshot.session.supplyRound, snapshot.activeItem.itemId)
    ...
    const event = { ...base, payload: { ...base.payload, item: snapshot.activeItem, requestId: snapshot.activeRequestId ?? `${snapshot.session.sessionId}:supply`, nextSupplyCursor: snapshot.suppliedNextCursor ?? snapshot.activeItem.itemId, ...(supplyRound === undefined ? {} : { supplyRound }) } } as ExtraTrainingEvent
    const count = snapshot.session.completedItemCount + 1
    return this.save({ ...snapshot, phase: 'answering', pendingEvents: [...snapshot.pendingEvents, attempt, event], session: { ...snapshot.session, excludeItemIds: [...snapshot.session.excludeItemIds, snapshot.activeItem.itemId], ...(supplyRound === undefined ? {} : { supplyRound }), completedItemCount: count, nextSupplyCursor: snapshot.suppliedNextCursor ?? snapshot.activeItem.itemId, status: 'running', endReason: null, endedAt: null, updatedAt: event.occurredAt }, updatedAt: event.occurredAt })
  }
