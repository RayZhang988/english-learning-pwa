...
describe('R13-C scene vocabulary practice runtime', () => {
  it('replays one durable scene error without changing R13-C feedback or target-only playback', async () => {
    const bank = await loadReleasedSceneBank(); const index = await loadReleasedReviewContentIndex() as ReviewContentIndex; const repository = new MemoryScenePracticeRepository(); let state = createWrongAnswerLibraryState(); const ids: string[] = []; let fail = true
  ...
  it('keeps scene evidence pending when confirmation-clear save fails, then replays idempotently', async () => {
    const bank = await loadReleasedSceneBank(); const index = await loadReleasedReviewContentIndex() as ReviewContentIndex; const repository = new MemoryScenePracticeRepository(); let state = createWrongAnswerLibraryState(); const ids: string[] = []
  ...
  it('does not create scene evidence for correct, selected-only, or exit snapshots', async () => {
    const bank = await loadReleasedSceneBank(); const index = await loadReleasedReviewContentIndex() as ReviewContentIndex; const sink: WrongAnswerEvidenceSink = { publish: async () => { throw new Error('unexpected evidence') } }; const review = { index, sink }
  ...
  it('emits one scene evidence for a feedback double-submit and none for a correct formal answer', async () => {
    const bank = await loadReleasedSceneBank(); const index = await loadReleasedReviewContentIndex() as ReviewContentIndex; let state = createWrongAnswerLibraryState(); const ids: string[] = []; const sink: WrongAnswerEvidenceSink = { async publish(evidence) { ids.push(evidence.eventId); state = applyWrongAnswerEvidence(state, evidence).state } }; const review = { index, sink }
  ...
  it('fails corrupt and drifted R13-C snapshots before any scene evidence handoff', async () => {
    const bank = await loadReleasedSceneBank(); const index = await loadReleasedReviewContentIndex() as ReviewContentIndex; let calls = 0; const review = { index, sink: { publish: async () => { calls += 1 } } }
