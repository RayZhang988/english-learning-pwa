...
- 08 `b878965` 已让完整 122 个正式口语候选可解析，其中包括 28 个
  `speaking-scene-quiz / scene-fixed-response`，没有过滤索引或回退固定题目。
- 原稳定失败命令现为 1 文件/1 项通过；真实 first-use 生产链路的口语首题正常进入，
  899/900 秒和 `finish-current-item → budget.completed` 完成。
- 新 09 外部 QA-011 验收 1 文件/7 项通过，覆盖三模块 cursor/exclude、耗尽恢复、
  错误 recovery 拒绝、重复幂等，以及听力播放/口语录音自然收尾。
