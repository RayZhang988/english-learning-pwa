...
| QA-008 | 新建 | S2 | 底部“训练”页四个公开入口全部进入“暂无可用训练”占位页 | 正式资产 `index-DQn2F3sQ.js` | 01 主责，02 公开入口契约协同；08 暂不返工 | 390×844 正式站黑盒：水平测试、词汇、听力、口语四项均停留 `/#/` 并显示同一占位正文 |
...
2. “训练 → 水平测试”必须进入真实 `#/assessment` 流程，不显示占位正文。
