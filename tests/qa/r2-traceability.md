...
| R2-14 | 用户在真实 iPhone 入口确认可自由选择 | 2026-07-27 用户在手机实际入口确认 R2 已修复；00 据此关闭 R2 | 用户亲自在实际入口确认自由选择；该证据不扩大为完整 iPhone 清单通过 | 通过 |

## 当前门禁

R2 的外部契约、正式部署、桌面真实浏览器、六种顺序、刷新、幂等、错误拒绝和窄屏
门槛均通过。2026-07-27 用户又在手机实际入口确认修复，00 已关闭 R2 并只激活 R3。
这不代表完整 iPhone 清单或第一版通过，后续仍按单项需求串行门禁执行。
