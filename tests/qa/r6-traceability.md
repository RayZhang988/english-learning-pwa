...
## R6.1 规则变更（取代原始 R6 时间预算规则）

2026-07-30 用户取消“额外训练每轮 15 分钟”的完成条件。下方 R6-002、R6-004、
R6-005 和测试模式记录保留为历史证据，但不再是当前产品规则。

- 每日必做词汇、听力、口语仍各为 900 秒有效训练，完全不受本次变更影响。
- 每日 3/3 后的额外训练为开放式：无目标时长、无剩余倒计时、无自动完成。
- 开放式训练继续记录前台有效秒数、完成题数、分数、供应游标和排除集合。
- 完成一题后继续供应；只有用户主动退出才暂停并保存，之后可恢复。
- 旧的未完成 900 秒额外会话恢复时迁移为开放式并保留已有进度；旧完成记录保留历史原貌。
- `learning.extra-training.budget.completed.v1` 只为读取旧事件保留，新会话不得发布或接受它。

R6.1候选版本：`ecc3cf6`，Pages run `30513807922`，正式资产
`index-0IPw8wq9.js`。
当前结论：**R6原需求已关闭；R6.1的外部契约、本地与正式站生产浏览器、完整工程门禁
全部通过。用户已确认其余开放式训练行为正常；当前只待“不显示累计有效练习时长”的
展示修补（`aa3e3a2`、Pages run `30514132712`、正式资产 `index-QfZz41h1.js`）
及其后续“累计完成右侧显示正确率”补充（`483c00f`、Pages run `30514414331`、
正式资产 `index-CCTvhVGM.js`）在实际入口确认后关闭。正确率只使用可评分题目，不可评分
结果不进入分母；底层可信计时记录继续保留。**
