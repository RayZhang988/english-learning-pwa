...
验收版本：本地候选 `7193aa0`。  
当前结论：**外部契约、本地与正式站生产浏览器、完整工程门禁通过；等待真实 iPhone。**
...
| R6-011 | 正式站版本、缓存更新及真实 iPhone Safari/主屏幕 App 的触摸、后台、锁屏、音频/录音行为必须单独验收 | Pages run `30428036715`、正式资产 `index-CSfrI9g9.js`；正式 URL 完整黑盒通过；真机按 `iphone-checklist.md` 执行 | 01/09/用户 | 待真机 |
...
- 本地生产浏览器实测30.124秒、正式站Pages run`30437466042`实测30.204秒进入
  `finish-current-item`并完成当前词汇题；正式资产为`index-Bq-OM850.js`，同一构建的
  普通15分钟R6黑盒继续通过。
