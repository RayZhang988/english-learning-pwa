...
**听力自然度门槛通过；正式“训练”页四个公开入口 4/4 落入占位页，第一版不通过。**
...
| QA-008 | S2 | 新建 | 正式资产 `index-DQn2F3sQ.js`；训练页水平测试、词汇、听力、口语 4/4 保持 `/#/` 并进入“暂无可用训练”占位页 |
...
| G2 训练衔接 | 不通过 | “今天”页真实 taskId 计划 3/3 通过，但底部“训练”页四个公开入口 4/4 未进入评估或模块路由，QA-008 打开 |
...
阶段性结论：**QA-008 已复现；公开训练入口门槛不通过，第一版不通过。**

下一步顺序：

1. 00 将 QA-008 派回 01 主责、02 公开入口契约协同；08 暂不返工。
2. 修复部署后，09 在正式站用真实 AbilityProfile、active-plan 和 taskId 回归四入口，
   同时确认“今天”页计划 3/3 不回归。
3. 再完成其余 iPhone 安装、离线、权限、后台、恢复和无障碍清单。
4. 只有 QA-008 关闭且完整真机门槛通过后，才开始连续 14 天记录。
