...
**自然单 voice 候选的正式自动化门槛通过；真实听感门槛仍未通过，第一版当前不通过。**

`QA-006` 听写竞态仍为已关闭。旧 one/two voice 自动化虽然全部通过，但用户真实
听感明确否决其多 voice、变调变速和逐句 utterance 方案，现登记 `QA-007`。自然度
优先候选已撤除这些机制并通过正式自动化，但尚未经过真实 iPhone/用户听音，不能
预先关闭体验问题。

## 版本与范围

- 当前正式功能版本：自然单 voice 候选 `724565b`
- 当前 09 验收同步版本：`98ffb1b`
...
全量为 77 个文件、291 项测试通过。`index-DQn2F3sQ.js` 已是 run
`30147986654` 部署的正式资产。
...
以上只能证明导致旧机械听感的代码机制已撤除。候选虽已部署且正式自动化门槛通过，
但真实 iPhone/用户还没有听过新版本；QA-007 保持打开。
...
| QA-007 | S2 | 待真机回归 | `724565b` / `98ffb1b` / run `30147986654` 正式自动化通过；真实 iPhone/用户自然度、连续性和可用性尚未确认 |
...
| G2 训练衔接 | 自动化通过 | 自然单 voice 正式候选通过真实 taskId 完成口语 fallback、听力、词汇和计划 3/3，`planCompleted=true` |
...
| G5 设备与语音 | 不通过 | 正式候选自动化只证明旧风险机制撤除和控制链路通过；真实 iPhone/用户听感未验，QA-007 未关闭 |
...
阶段性结论：**正式自动化门槛已通过；听力真实体验门槛仍未通过，第一版当前不通过。**

下一步顺序：

1. 用户在真实 iPhone 上听完整对话和单句，只有自然度、清晰度、连贯性和可用性
   通过后关闭
   QA-007。
2. 再完成其余 iPhone 清单，最后开始连续 14 天记录。
