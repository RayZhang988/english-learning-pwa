...
**QA-011、QA-012 与 QA-013 均已关闭；R3 仍待新版 900 秒预算的完整 iPhone 真机验收。**
...
通过；随后用户在不清数据的原 iPhone 计划上实际完成听力 38 秒与口语 3 个固定
提示，均未再出现 JSON-portable 错误，QA-013 已关闭。现在继续 `iphone-checklist.md`
的锁屏、后台、真实媒体生命周期和新版 900 秒自然收尾。
...
用户随后使用原 iPhone IndexedDB 实际完成听力和口语，未清除网站数据，也未重做
水平测试；听力完成页显示 38 秒，口语完成页显示 3 个固定提示。QA-013 据此关闭。
两者仍属于旧无预算计划，不能作为新版每模块 900 秒预算的通过证据。
